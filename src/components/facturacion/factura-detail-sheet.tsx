...
interface FacturaDetailSheetProps {
  ...
  onAddPago: (pago: Omit<PagoFactura, "id">) => void
}
...
export function FacturaDetailSheet({
  ...
  const [pagoFormOpen, setPagoFormOpen] = React.useState(false)

  if (!factura) return null

  const facturaPagos = pagos.filter((p) => p.factura_id === factura.id)
  const totalPagado = facturaPagos.reduce((sum, p) => sum + p.importe, 0)
        ...
        <SheetContent side="right" className="sm:max-w-lg overflow-y-auto">
          ...
          <div className="px-4 space-y-4">
            ...
            <div>
              ...
              {facturaPagos.length > 0 ? (
                <div className="space-y-2">
                  {facturaPagos.map((pago) => (
                    ...
                    >
                      <div>
                        <p className="font-medium">{formatCurrency(pago.importe)}</p>
                        <p className="text-xs text-muted-foreground">
                          {formatDate(pago.fecha)} · {METODO_PAGO_LABELS[pago.metodo_pago]}
                        </p>
                        {pago.notas && (
              ...
              {/* Payment summary */}
              <div className="mt-3 rounded-lg bg-muted/50 p-3 space-y-1 text-sm">
                ...
                <div className="flex justify-between border-t pt-1">
                  ...
                  <span className={`font-bold ${saldoPendiente > 0 ? "text-amber-600" : "text-green-600"}`}>
